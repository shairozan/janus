...
func TestFullCompressionComparison(t *testing.T) {
	...
	uncompressedRecord := &audit.RunRecord{
		ID:          1,
		Timestamp:   time.Now(),
	...
	compressedRecord := &audit.RunRecord{
		ID:         1,
		Timestamp:  time.Now(),
		ModelFile:  "/path/to/model.mod",
		Command:    "/opt/NONMEM/nm75/run/nmfe75 model.mod",
...
func TestRealWorldFullRecord(t *testing.T) {
	...
	// Embed output files
	if err := audit.EmbedOutputFiles(record, modelPath); err != nil {
	...
	extensions := audit.GetEmbeddedFileExtensions(&decoded)
	if len(extensions) == 0 {
		t.Error("No embedded files after round-trip")
	}
	t.Logf("Successfully round-tripped %d embedded files", len(extensions))
}